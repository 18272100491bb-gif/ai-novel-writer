...
重要：总字数（包含所有标签字段）不得超过500字。严格压缩每个字段，不要冗余描写。`,
	...
	FactCheck: `你是一位严谨的小说验收员，负责检查小说章节中的客观事实矛盾和写作质量问题。
...
{{.OutlineConstraints}}{{.Memory}}【待验收章节】
{{.ChapterContent}}
...
注意：文风、节奏、详略取舍、剧情合理性等主观问题不属于事实错误，拿不准时一律判 PASS。

=== 维度二：情绪冗余检测（Gate 7） ===
逐句判断当前句子是否为本场景已积累信息提供了新的具体信息。检测规则：
7.1 同一情绪状态被多次显式声明，后文无新信息
7.2 抽象情绪词包裹具体动作，拆开写后冗余（如"悲伤的浪潮涌起"+"泪水模糊了视线"→前者FAIL）
7.3 场景描写完成后，追加一句情绪总结（如"桌上只有一碗冷粥。他感到一阵凄凉。"→后句FAIL）
7.4 已通过动作/对话/环境间接表达的情绪，又用心理活动再写一遍
7.5 通用化情绪比喻，无场景特异性（如"像打翻了五味瓶"→FAIL）
7.6 角色已明确表态后，追加心理活动解释
7.7 跨段重复同一情绪点（无新触发事件时再次声明）

注意：排比等刻意重复为豁免，相似情绪因新触发事件而再次激发也为豁免。

=== 维度三：修辞选择自觉（Gate 8） ===
检查修辞手法是否与当前场景的情感基调、叙事节奏相契合：
8.1 修辞是否与场景格调一致（欢快场景用阴暗修辞，或反之，则为不恰当）
8.2 修辞是否过于浮夸或过于平淡（高潮场景用平淡修辞，或平淡场景用浮夸修辞）
8.3 同一段落中修辞密度是否过高（连续3句以上堆砌比喻/排比，造成阅读疲劳）
8.4 角色专属修辞是否符合其身份和性格（农民角色用学术修辞，或反之，则为不恰当）
8.5 修辞是否服务于叙事目的（单纯为了"写好看"而不推进情节或塑造氛围，则为冗余）

注意：修辞品味因人而异，拿不准时一律判 PASS。
...
result为PASS仅当三个维度全部PASS，任一维度FAIL则result为FAIL。各维度独立判断，请给出各自的问题列表。`,
...
6. 请严格以JSON格式输出，不要添加任何额外文字`,
	...
	OutlineParse: `你是一位严谨的小说设定分析员。请从完整故事大纲中提取角色、世界条目、组织、伏笔等设定信息，填入对应的JSON结构。

【完整大纲】
{{.FullSynopsis}}

请以JSON格式返回，字段说明：
- characters: 大纲中出现的所有角色（含配角），每个角色包含 name, age, appearance, personality, background, motivation, abilities, notes（注：age/appearance等如果没有明确信息就留空字符串）
- worldview: 大纲中涉及的世界观设定（修炼体系、地理、历史事件、特殊规则等），每项包含 category（分类：geography地理/faction势力/rule规则/history历史/other其他，根据内容选最贴合的）, name, description, tags（标签，逗号分隔）
- organizations: 大纲中出现的组织/门派/势力，每项包含 name, type（门派/帮会/国家/家族等）, description, members（成员名列表）
- foreshadows: 大纲中埋伏的剧情线/悬念/伏笔，每项包含 name, description, plant_chapter（埋设章节，数字）, target_chapter（预期回收章节，数字，0表示未知）
- relations: 角色与角色、角色与组织之间的关系链，每项包含 source（源名称）, target（目标名称）, label（关系标签，如师徒/敌对/父子/盟友等）

注意：
1. 只从【完整大纲】中提取明确信息，不要自行编造
2. 不修改原文，只提取和分类
3. 输出严格JSON，不要额外文字
...
{
  "new_memories": [
    {"content": "记忆内容描述", "category": "分类", "position": 段落序号, "importance": 重要度(0-1.0)}
  ],
  "updates": [
