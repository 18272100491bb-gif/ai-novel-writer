...
func generateChapterContentStream(ctx context.Context, apiCfg *APIConfig, cfg *Config, state *Progress, idx int, settings *ProjectSettings, extraWritingConstraints string, logger *LogBroadcaster) (string, error) {
	...
	// 写作人格永远第一，CorePrompt 只是附加项目指导
	systemPrompt := loadPersonaPrompt(lang)
	if state.CorePrompt != "" {
		systemPrompt += "\n\n【项目创作指导】\n" + state.CorePrompt
	}
...
func syncMemoryAfterChapter(ctx context.Context, apiCfg *APIConfig, cfg *Config, state *Progress, idx int, progressPath string, logger *LogBroadcaster) {
	...
	go func() {
		...
		if !Mem0Health() {
			logger.Emit("mem0_status", map[string]any{
				"status": "degraded",
				"msg":    "记忆服务暂不可用，已降级到原生记忆",
			})
			return
		}
		written := 0
		for _, nm := range newMemories {
			...
			if Mem0Add(ctx, progressPath, ch.Num, nm.Content, nm.Category, imp, nil) {
				written++
			}
		}
		if written > 0 {
	...
	// 选角：每20章更新实体权重，影响后续搜索
	if ch.Num > 0 && ch.Num%20 == 0 {
		weights := Mem0UpdateWeights(progressPath)
		if len(weights) > 0 {
			logger.Emit("mem0_status", map[string]any{
				"status":      "rerank",
				"msg":         "选角完成，更新实体检索权重",
...
// loadPersonaPrompt 读取 persona.txt 覆盖，否则用硬编码默认
func loadPersonaPrompt(lang string) string {
	personaPath := "persona.txt"
	if data, err := os.ReadFile(personaPath); err == nil && len(data) > 0 {
		return string(data)
	}
	return SystemPromptFor(lang, "author_default")
}
