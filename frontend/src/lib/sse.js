...
export function connectSSE() {
  ...
  eventSource.addEventListener('mem0_status', e => {
    const d = JSON.parse(e.data);
    if (d.status === 'ok') {
      addToast(translate('toast.mem0Ok', { count: d.count, chapter: d.chapter }), 'success');
    } else if (d.status === 'degraded') {
      addToast(translate('toast.mem0Degraded'), 'warning');
    } else if (d.status === 'rerank') {
      addToast('选角已完成，实体权重已更新', 'info');
    }
  });
