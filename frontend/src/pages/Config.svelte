<script>
  ...
  // 自动加载写作人格
  $: if (!personaLoaded && !personaSaving) {
    loadPersona();
  }
  ...
  async function loadPersona() {
    try {
      ...
      localPersona = res.persona;
      personaLoaded = true;
    } catch (e) {
      addToast('无法加载写作人格', 'error');
    }
  }
  ...
  async function savePersona() {
    ...
    personaSaved = false;
    try {
      await api('PUT', '/api/persona', { persona: localPersona });
      personaSaved = true;
      addToast('写作人格已更新', 'success');
    } catch (e) {
...
<div class="space-y-3">
  ...
  <div class="card bg-base-200 shadow-sm">
    <div class="card-body py-4 gap-3">
      <h3 class="card-title text-sm">写作人格（系统提示词）</h3>
      <p class="text-xs text-base-content/50">这是LLM写作时的身份设定，人格永远优先于项目级 CorePrompt。编辑后全局生效。</p>
      <div class="space-y-2">
        <textarea
          class="textarea textarea-bordered w-full h-48 font-mono text-xs leading-relaxed"
          bind:value={localPersona}
          disabled={personaSaving}
        ></textarea>
        <div class="flex gap-2 items-center">
          <button class="btn btn-primary btn-sm" on:click={savePersona} disabled={personaSaving}>
            {personaSaving ? '保存中…' : '保存人格'}
          </button>
          <button class="btn btn-ghost btn-sm" on:click={loadPersona}>恢复默认</button>
          {#if personaSaved}
