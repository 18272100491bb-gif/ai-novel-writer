...
def _get_data_dir() -> str:
    dir_ = os.environ.get("MEM0_DATA_DIR", "")
    if not dir_:
        raise RuntimeError("MEM0_DATA_DIR not set")
    return dir_


def get_db(project_id: str) -> MemoryDB:
    if project_id not in _DBS:
        db_dir = os.path.join(_get_data_dir(), project_id, ".mem0")
...
@app.post("/api/merge/{project_id}")
def merge_memories(project_id: str):
    """合并相似记忆（dedup by embedding similarity）"""
    db = get_db(project_id)
    try:
        result = db.merge_similar()
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
if __name__ == "__main__":
    ...
    os.environ["MEM0_DATA_DIR"] = args.data_dir
    print(f"[mem0] Starting on port {args.port}, data dir: {args.data_dir}", flush=True)
