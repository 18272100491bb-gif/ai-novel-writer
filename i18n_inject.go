...
// 组合热记忆（原生 MemoryEntry）+ 冷记忆（Mem0 检索）
func buildMemoryForLang(state *Progress, idx int, lang string) string {
	en := NormalizeLanguage(lang) == LangEN

	// 1. 热记忆（原生）
	hotBlock := buildHotMemoryBlock(state, idx, en)

	// 2. 冷记忆（Mem0 检索）
	coldBlock := buildColdMemoryBlock(state, idx)

	if hotBlock == "" && coldBlock == "" {
		return ""
	}

	return hotBlock + coldBlock
}

// buildHotMemoryBlock 生成原生热记忆块
func buildHotMemoryBlock(state *Progress, idx int, en bool) string {
...
// buildColdMemoryBlock 从 Mem0 检索冷记忆
func buildColdMemoryBlock(state *Progress, idx int) string {
	if state.ProjectDir == "" {
		return ""
	}
