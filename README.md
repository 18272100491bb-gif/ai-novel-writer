...
AI 写长篇小说最大的问题是"记不住前面写了什么"。原版只靠生成 prompt 里的历史摘要压缩，章数一多细节全丢。

我们引入 Mem0 侧车服务，构建**热冷双轨记忆架构**：

- **热记忆**：保留原版 progress.json 逐章片段，即时生效
- **冷记忆**：Mem0 + BGE 本地向量检索，每 5 章触发一次融合检索（BM25 + embedding + 实体权重 + 邻近加权），跨章节召回被历史摘要压缩掉的关键细节

同时每 20 章自动重算实体权重——让 AI 始终聚焦当前剧情线的核心角色，而不是被出场一两次的路人分散注意力。Mem0 服务不可用时自动降级回原生记忆，不影响生成流程。

### 🎭 三层写作人格

AI 的文笔问题，根源往往不是"不会写"，而是"不知道自己是谁"。
...
从 [Releases](https://github.com/18272100491bb-gif/ai-novel-writer/releases) 下载对应平台二进制，或自行编译：
...
- **后端**：Go（单文件二进制）
- **前端**：Svelte 4 + Vite
- **记忆检索**：Mem0 + BAAI/bge-small-zh-v1.5（本地向量，不调外部 API）
