...
func main() {
	...
	authPwd := os.Getenv("AUTH_PASSWORD")
	if authPwd != "" {
		fmt.Println(" [系统] 已启用访问密码验证")
	}

	startWebServer(apiCfg, apiCfgPath, cfg, state, settings, skills, sessionsDir, logger, port, progDir, version, authPwd)
}
